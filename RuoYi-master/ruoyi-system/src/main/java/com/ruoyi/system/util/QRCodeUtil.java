...
public class QRCodeUtil {
    ...
    public static BufferedImage createImage(String charSet, String content, int qrWidth, int qrHeight){
        ...
        try {
            bitMatrix = new MultiFormatWriter().encode(content, BarcodeFormat.QR_CODE,qrHeight , qrHeight, // 修改二维码底部高度
                    hints);
        ...
        BufferedImage image = new BufferedImage(qrWidth, height, BufferedImage.TYPE_INT_RGB);
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                image.setRGB(x, y, bitMatrix.get(x, y) ? 0xFF000000 : 0xFFFFFFFF);
            }
        }
        //insertLogoImage(image,compressLogo("C:\\Users\\Administrator\\Desktop/123.jpg",100,200),100,200);
        // addBottomFont(image,"0718-8222205",1);
        return image;
    }
    ...
    public static void insertLogoImage(BufferedImage source,Image logo,int logoWidth,int logoHeight){
        ...
        graph.drawImage(logo, qrHeight, 0, logoWidth, logoHeight, null);
        Shape shape = new RoundRectangle2D.Float(qrHeight, 0, logoWidth, qrHeight, 6, 6);
    ...
    public static BufferedImage addBottomFont(BufferedImage source, String text,int step) {
        ...
        int qrWidth = source.getHeight();
        System.out.println("二维码的宽度"+qrWidth);
        int qrHeight = source.getHeight();
        System.out.println("二维码的高度"+qrHeight);
        BufferedImage textImage = textToImage(text, qrWidth, 25,16);
        Graphics2D graph = source.createGraphics();
        //开启文字抗锯齿
        ...
        Image src = textImage;
        graph.drawImage(src, qrWidth-30, 100, 150, height, null);
        graph.dispose();
        return  source;
    }
